...
/* led handle */
const led_handle_ops_t g_led1_adapter_ops = {.pf_drv_init  = drv_led_init,
...
mp3dec_t                                  mp3d;
__attribute__((section(".ram_dma_buffers"),
               aligned(4))) mp3d_sample_t pcm[MINIMP3_MAX_SAMPLES_PER_FRAME];
...
void StartDefaultTask(void *argument)
{
    ...
    for(;;)
    {
        ...
        int                 buf_size          = (int)Array_size;
        int                 total_frames      = 0;
        int                 max_frames_to_log = 10; // 只打印前10帧

        while(buf_size > 0)
        {
            int samples = mp3dec_decode_frame(&mp3d, input, buf_size, pcm,
                                              &info);
            ...
            if(samples > 0)
            {
                ...
                // ---- 只打印前 10 帧的 PCM 数据 ----
                if(total_frames <= max_frames_to_log)
                {
                    int16_t *p      = (int16_t *)pcm; // minimp3输出是int16_t
                    int num_samples = (samples > 10)
                                          ? 10
                                          : samples; // 每帧最多打10个采样点

                    // 用logInfo打印帧号
                    char msg[128];
                    sprintf(msg, "Frame %d (samples=%d): ", total_frames,
                            samples);
                    logInfo("%s", msg);

                    // 逐个打印采样值（立体声交错 L R L R ...）
                    for(int i = 0; i < num_samples; i++)
                    {
                        // 用logInfo逐个数打印，或者你可以拼接成一个字符串再打印
                        sprintf(msg, "  %d", p[i]);
                        logInfo("%s", msg);
                    }
                }
