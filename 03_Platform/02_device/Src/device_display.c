 ...
 *
 *@brief              :   Display device implementation — ST7789 via SPI.
 *
 *@version            :   V2.0
 *
...
{
    if (NULL == p_font || NULL == p_str) { return PLATFORM_ERR_PARAM; }
...
{
    if (NULL == p_pixels) { return PLATFORM_ERR_PARAM; }
...
platform_err_t device_display_draw_dec(const front_def_t *p_font,
                                        ...
                                        uint16_t f_color, uint16_t b_color)
{
    if (NULL == p_font) { return PLATFORM_ERR_PARAM; }
    st7789_state_t ret = g_lcd.pf_draw_dec(&g_lcd, p_font, x, y, value,
                                            f_color, b_color);
    return (ST7789_OK == ret) ? PLATFORM_ERR_OK : PLATFORM_ERR_HW;
}
...
{
    if (NULL == p_font) { return PLATFORM_ERR_PARAM; }
    st7789_state_t ret = g_lcd.pf_draw_hex(&g_lcd, p_font, x, y, value,
                                            f_color, b_color);
    return (ST7789_OK == ret) ? PLATFORM_ERR_OK : PLATFORM_ERR_HW;
}
