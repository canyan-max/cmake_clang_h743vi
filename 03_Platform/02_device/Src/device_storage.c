 ...
 *
 *@brief              :   Storage device implementation — AT24C02 via I2C.
 *
 *@version            :   V2.0
 *
...
platform_err_t device_storage_init(const void *p_iic_ops,
                                    ...
                                    uint8_t     dev_adr)
{
    if (NULL == p_iic_ops) { return PLATFORM_ERR_PARAM; }
...
platform_err_t device_storage_write_page(uint16_t mem_adr,
                                          ...
                                          uint32_t timeout)
{
    if (NULL == p_data || 0U == size) { return PLATFORM_ERR_PARAM; }
    at24_state_t ret = g_eeprom.pf_write_page(&g_eeprom, mem_adr, p_data, size, timeout);
    return (AT24_OK == ret) ? PLATFORM_ERR_OK : PLATFORM_ERR_HW;
}
                                                ...
                                                uint32_t timeout)
{
    ...
    while (0U != left_size)
    {
        ...
        ret = g_eeprom.pf_write_page(&g_eeprom, current_adr, p_cursor, chunk_size, timeout);
        if (AT24_OK != ret) { return PLATFORM_ERR_HW; }
        current_adr += chunk_size;
        p_cursor    += chunk_size;
        left_size   -= chunk_size;
        if (0U == left_size) { break; }
        ...
        while ((HAL_GetTick() - start) <= 5U)
        {
            if (AT24_OK == g_eeprom.iic_ops->pf_iic_dev_isready(
                   g_eeprom.iic_ops->p_iic_handle, g_eeprom.dev_adr, 1U, 0U))
            {
                timeout_flag = 0U;
                break;
            }
        }
        if (timeout_flag) { return PLATFORM_ERR_TIMEOUT; }
    }
...
{
    if (NULL == p_data || 0U == size) { return PLATFORM_ERR_PARAM; }
    at24_state_t ret = g_eeprom.pf_read_bytes(&g_eeprom, mem_adr, p_data, size, timeout);
    return (AT24_OK == ret) ? PLATFORM_ERR_OK : PLATFORM_ERR_HW;
}
