...
def cmake_files(root, layered):
    service_subdir = "add_subdirectory(02_Service)\n" if layered else ""
    write(root, "CMakeLists.txt", f"""cmake_minimum_required(VERSION 3.22)
project(mcu_layered_skeleton LANGUAGES C)

set(CMAKE_C_STANDARD 11)
set(CMAKE_C_STANDARD_REQUIRED ON)
set(CMAKE_C_EXTENSIONS ON)

add_subdirectory(00_Board)
add_subdirectory(04_Platform/01_common)
add_subdirectory(04_Platform/03_mcu_interface)
add_subdirectory(05_Impl/01_mcu)
add_subdirectory(04_Platform/02_bsp)
{service_subdir}add_subdirectory(01_App)
""")
    write(root, "00_Board/CMakeLists.txt", """add_library(board_resources INTERFACE)
target_include_directories(board_resources INTERFACE
    ${CMAKE_CURRENT_SOURCE_DIR}/Inc
)

add_library(board_target_binding INTERFACE)
target_include_directories(board_target_binding INTERFACE
    ${CMAKE_CURRENT_SOURCE_DIR}/Inc
)
""")
    write(root, "04_Platform/01_common/CMakeLists.txt", """add_library(platform_common INTERFACE)
target_include_directories(platform_common INTERFACE
    ${CMAKE_CURRENT_SOURCE_DIR}/Inc
)
""")
    write(root, "04_Platform/03_mcu_interface/CMakeLists.txt", """add_library(mcu_interface INTERFACE)
target_include_directories(mcu_interface INTERFACE
    ${CMAKE_CURRENT_SOURCE_DIR}/Inc
)
target_link_libraries(mcu_interface INTERFACE platform_common)
""")
    write(root, "05_Impl/01_mcu/CMakeLists.txt", """add_library(impl_mcu STATIC
    Src/mcu_gpio.c
)
target_link_libraries(impl_mcu PRIVATE
    mcu_interface
    board_resources
    board_target_binding
)

# Add the selected vendor SDK target here as another PRIVATE dependency.
""")
    write(root, "04_Platform/02_bsp/CMakeLists.txt", """add_library(bsp_interface INTERFACE)
target_include_directories(bsp_interface INTERFACE
    ${CMAKE_CURRENT_SOURCE_DIR}/Inc
)
target_link_libraries(bsp_interface INTERFACE platform_common)

add_library(bsp STATIC
    Src/bsp_status_led.c
)
target_link_libraries(bsp
    PUBLIC bsp_interface
    PRIVATE board_resources mcu_interface
)
""")
    app_dependency = "target_link_libraries(app PRIVATE service)\n" if layered else ""
    write(root, "01_App/CMakeLists.txt", f"""add_library(app STATIC
    Src/app_main.c
)
...
{app_dependency}""")
    if layered:
        write(root, "02_Service/CMakeLists.txt", """add_library(service STATIC
    Src/service_app.c
)
target_include_directories(service PUBLIC
    ${CMAKE_CURRENT_SOURCE_DIR}/Inc
)
target_link_libraries(service PRIVATE bsp)
""")
...
def main():
    ...
    if a.kind == "cmake": cmake_files(root, a.profile == "layered")
