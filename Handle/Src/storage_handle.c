                                ...
                                uint8_t           dev_adr)
{
    ...
    return p_handle->p_ops->pf_drv_init(p_handle->p_drv, max_byte_addr,
                             ...
                             uint32_t          timeout)
{
    if(NULL == p_handle || NULL == p_data || 0 == size ||
       NULL == p_handle->p_ops || NULL == p_handle->p_ops->pf_write_page)
    {
        return 1;
    }
    return p_handle->p_ops->pf_write_page(p_handle->p_drv, mem_adr, p_data,
                                          size, timeout);
}
...
{
    if(NULL == p_handle || NULL == p_data || 0 == size ||
       NULL == p_handle->p_ops || NULL == p_handle->p_ops->pf_write_cross_page)
    {
        return 1;
    }
...
{
    if(NULL == p_handle || NULL == p_data || 0 == size ||
       NULL == p_handle->p_ops || NULL == p_handle->p_ops->pf_read_bytes)
    {
        return 1;
    }
    return p_handle->p_ops->pf_read_bytes(p_handle->p_drv, mem_adr, p_data,
                                          size, timeout);
}
