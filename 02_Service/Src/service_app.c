...
platform_err_t service_app_init(void)
{
    ...
    if(PLATFORM_ERR_OK != device_display_fill_screen(0x0000U))
    {
        return PLATFORM_ERR_HW;
    }
    if(PLATFORM_ERR_OK != device_camera_init())
    {
        return PLATFORM_ERR_HW;
    }
    if(PLATFORM_ERR_OK != device_camera_start())
    {
        return PLATFORM_ERR_HW;
    }
...
{
    service_osd_render();
    device_display_draw_image(0U, OSD_BAR_HEIGHT, 240U, 240U - OSD_BAR_HEIGHT,
