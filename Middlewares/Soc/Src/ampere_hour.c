                                        ...
                                        float availcapacity)
{
    ...
    real_detal_availcapacity = (dsg_instcurrent * SAMINTERVALTIME / 3600 / availcapacity) * 100;	//mAh
 ...
 */
static uint8_t calculate_ranks(float temp_value, float dsg_ratio, \
                               float *avaicap_value, float *temp_dsg_ratio)
{
    uint8_t i = 0 ;
    ...
    else
    {
        ...
        {
            if(dsg_ratio > dsg_ratio_table[i] && dsg_ratio < dsg_ratio_table[i+1])
            {
                n1 = i;
                n2 = i+1;
                break;
...
static float calculate_availcap(float temp_value, float dsg_ratio)
{
   ...
   float high_temp_avai_cap = 0.0;
    if(calculate_ranks(temp_value, dsg_ratio, avai_cap_value, temp_dsg_ratio) == 0)
    {
        return 0.0; 
    }
    ...
    low_temp_avai_cap = (high_dsg_ratio_low_temp_avai_cap - low_dsg_ratio_low_temp_avai_cap) / \
                        (high_dsg_ratio - low_dsg_ratio+0.0001f) * (dsg_ratio - low_dsg_ratio) + \
                         low_dsg_ratio_low_temp_avai_cap;

    high_temp_avai_cap = (high_dsg_ratio_high_temp_avai_cap - low_dsg_ratio_high_temp_avai_cap) / \
                         (high_dsg_ratio - low_dsg_ratio+0.0001f) * (dsg_ratio - low_dsg_ratio) + \
...
float get_detal_capacity (float current , float bat_cap_max , int16_t bat_temp)
{
    ...
    detal_cap = calculate_real_dsgcapacity((float)(current/1000.0f),avail_capacity);
    return detal_cap;
}
