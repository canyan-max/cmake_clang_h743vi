...
#ifndef SOC_DEBUG
    float detal_cap_mah = get_detal_capacity(i_current, g_full_max_capacity,
                                             tempera);
#else
    detal_cap_mah = get_detal_capacity(i_current, g_full_max_capacity, tempera);
#endif
                                ...
                                uint16_t volt_diff)
{
    ...
    else if((0x00 == ov_flg) && 1 == max_calib_flg)
    {
        max_calib_flg = 0;
    }
...
void soc_init(uint8_t  pre_soc,
              ...
              uint16_t bat_capcity)
{
    float soc_temp_value   = ocv_table_startup_soc((volt / 1000.0f),
                                                   tempera); // soc 0.0 ~ 1.0
    ...
    g_old_normal_cap     = g_normal_cap;
    g_full_max_capacity  = bat_capcity *
                           g_soh; //  the battry max capcity uinit : mah
